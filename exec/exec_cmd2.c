...
/*   Updated: 2024/09/14 23:58:36 by ibes-sed         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    if (errno == ENOENT)
        (write(2, "No such file or directory 1\n", 29), exit(127));
    if (errno == EACCES)
...
t_arg_node *replace_node_args(t_arg_node *current, char **expanded_args, int count)
{
	t_arg_node  *prev;
	t_arg_node  *new_node;
