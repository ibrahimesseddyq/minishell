...
/*   Updated: 2024/10/29 08:02:46 by ibes-sed         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	create_and_handle_heredoc(t_redir_list *redirections, t_lst *lst)
{
	...
	while (current_heredoc)
	{
		if (current_heredoc->redir->type == NODE_HEREDOC)
		{
			...
			*(current_heredoc->redir->fd_heredoc_rd) = -1;
			if (write_heredoc_to_file(delimiter, heredoc_file,
					lst, current_heredoc->redir->fd_heredoc_rd) != 0)
	...
	return (signal(SIGINT, handle_sig), signal(SIGQUIT, handle_sig), 1);
}
