...
ls | wc -l | xargs echo ok

yes "no" | head -n 10 ok

/bin/echo -e "\033[33mHello, world!\033[0m" ok

/bin/echo '\1$42' > tmp_redir_out ok
ls -la | grep tmp_redir_out | awk '{print $1 $2}'
cat -e tmp_redir_out
rm tmp_redir_out

sleep 0 | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat ok
...
grep 1 < filedoesnotexist | echo 1 | grep 1 | head -1 | cat | ls ok

/usr/bin/env -i bash -c "env" | wc -l ok 
